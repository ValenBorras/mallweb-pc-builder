...
export const CATEGORIES: Record<CategoryKey, Category> = {
  ...
  mouse: {
    ...
    searchKeywords: ['mouse', 'raton', 'gamer'],
    required: false,
    order: 10,
  ...
  keyboard: {
    ...
    icon: '⌨️',
    description: 'Periférico de entrada',
    searchKeywords: ['teclado', 'keyboard', 'gamer'],
    required: false,
    order: 12,
    parentCategory: 'peripherals',
  },
  fans: {
    ...
    shortName: 'Fans',
    icon: '🌀',
    description: 'Ventiladores para gabinete',
    searchKeywords: ['ventilador', 'fan', 'cooler'],
    required: false,
    order: 14,
    parentCategory: 'peripherals',
  },
  peripherals: {
    ...
    searchKeywords: ['perifericos', 'accesorios', 'gamer'],
    required: false,
    order: 14,
    subCategories: ['mouse', 'headphones', 'keyboard', 'fans', 'peripherals'], // Order: Mouse, Auriculares, Teclado, Todos
  },
};
...
export function isCoolerRequired(cpuIncludesCooler?: boolean): boolean {
  // If CPU explicitly includes a cooler, external cooler is optional
  if (cpuIncludesCooler === true) {
    return false;
  }
  
  // In all other cases (false or undefined), cooler is REQUIRED
  // If CPU info is unknown or CPU does NOT include a cooler, external cooler is required
  // IMPORTANT: If CPU doesn't say it includes a cooler, assume it does NOT and require one
  return true;
}